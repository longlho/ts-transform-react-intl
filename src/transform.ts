...
export type Extractor = (msgId: string, messages: MessageDescriptor, filename?: string) => void

export type InterpolateNameFn = (sourceFileName: string, msg: MessageDescriptor) => string
...
function findMacroHook(
    ...
    let hook = ''
    if (ts.isImportDeclaration(node) && trimSingleQuote(node.moduleSpecifier.getText(sf)) === macroModuleSpecifier) {
...
function isMacroExpression(node: ts.Node, sf: ts.SourceFile, hook: string): node is ts.CallExpression & boolean {
